...
def reserved_value(x_tenant_id: str = Header()):
    ...
    The join is tenant-scoped so a reservation is only ever priced using an
    item row belonging to the same tenant.
    """
    rows = fetch_all(
...
async def import_snapshot(payload: dict, x_tenant_id: str = Header()):
    ...
    written, and all updates run inside a single transaction, so a malformed
    entry or a failed write leaves no entry applied.
    """
