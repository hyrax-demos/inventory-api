...
def test_release_reservation_is_reflected_on_next_stock_read(client, fake_db):
    ...
    assert client.get("/items/WIDGET/stock", params=params, headers=headers).json()["quantity"] == 5
    resp = client.post("/reservations/order-1/release", headers=headers)
    assert resp.status_code == 200
    assert client.get("/items/WIDGET/stock", params=params, headers=headers).json()["quantity"] == 8
