...
def test_release_reservation_refreshes_cached_stock(client, fake_db):
    # End-to-end through HTTP with the real in-process cache: the first GET
    # fills the cache, the release must invalidate the exact key GET reads,
    # so the second GET returns the restored quantity rather than a stale one.
    fake_db.add_item(sku="GADGET", warehouse_id="w1", quantity=10, tenant_id="tenant-a")
    headers = {"X-Tenant-Id": "tenant-a"}
    reserve = client.post(
        "/items/reserve",
    ...
    stock_url = "/items/GADGET/stock"
    params = {"warehouse_id": "w1"}
    before = client.get(stock_url, params=params, headers=headers)
    assert before.status_code == 200
    assert before.json()["quantity"] == 6

    resp = client.post("/reservations/order-7/release", headers=headers)
    assert resp.status_code == 200

    after = client.get(stock_url, params=params, headers=headers)
    assert after.status_code == 200
    assert after.json()["quantity"] == 10


def test_repeat_release_does_not_touch_cached_stock(client, fake_db):
    fake_db.add_item(sku="GADGET", warehouse_id="w1", quantity=10, tenant_id="tenant-a")
    fake_db.add_reservation(
        order_id="order-8",
        tenant_id="tenant-a",
    ...
    assert (
        client.post("/reservations/order-8/release", headers=headers).status_code == 200
    )
    assert (
        client.get(stock_url, params=params, headers=headers).json()["quantity"] == 14
    )

    # A repeat release is a 404 no-op: stock is not restored again and the
    # value served for the SKU stays the correct one.
    assert (
        client.post("/reservations/order-8/release", headers=headers).status_code == 404
    )
    assert (
        client.get(stock_url, params=params, headers=headers).json()["quantity"] == 14
    )
...
def test_release_for_one_tenant_leaves_other_tenant_stock_intact(client, fake_db):
    fake_db.add_item(sku="GADGET", warehouse_id="w1", quantity=10, tenant_id="tenant-a")
    fake_db.add_item(sku="GADGET", warehouse_id="w1", quantity=7, tenant_id="tenant-b")
    fake_db.add_reservation(
        order_id="order-9",
    ...
    resp = client.post(
        "/reservations/order-9/release", headers={"X-Tenant-Id": "tenant-a"}
    )
    assert resp.status_code == 200

    b = client.get(
        "/items/GADGET/stock", params=params, headers={"X-Tenant-Id": "tenant-b"}
    )
    assert b.status_code == 200
    assert b.json()["quantity"] == 7
    assert [i["quantity"] for i in fake_db.items] == [13, 7]
