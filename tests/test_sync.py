...
def test_release_reservation_is_reflected_on_next_stock_read(client, fake_db):
    ...
    fake_db.add_item(sku="WIDGET", warehouse_id="w1", quantity=5, tenant_id="tenant-a")
    fake_db.add_reservation(order_id="order-1", tenant_id="tenant-a", sku="WIDGET", warehouse_id="w1", quantity=3)
    before = client.get("/items/WIDGET/stock", params={"warehouse_id": "w1"}, headers=headers)
    assert before.json()["quantity"] == 5
    resp = client.post("/reservations/order-1/release", headers=headers)
    assert resp.status_code == 200
    after = client.get("/items/WIDGET/stock", params={"warehouse_id": "w1"}, headers=headers)
    assert after.json()["quantity"] == 8


def test_reserve_then_release_round_trips_stock_read(client, fake_db):
    headers = {"X-Tenant-Id": "tenant-a"}
    fake_db.add_item(sku="WIDGET", warehouse_id="w1", quantity=10, tenant_id="tenant-a")
    params = {"warehouse_id": "w1"}
    assert client.get("/items/WIDGET/stock", params=params, headers=headers).json()["quantity"] == 10
    client.post(
        "/items/reserve",
        json={"sku": "WIDGET", "warehouse_id": "w1", "quantity": 4, "order_id": "order-1"},
        headers=headers,
    )
    assert client.get("/items/WIDGET/stock", params=params, headers=headers).json()["quantity"] == 6
    client.post("/reservations/order-1/release", headers=headers)
    assert client.get("/items/WIDGET/stock", params=params, headers=headers).json()["quantity"] == 10
