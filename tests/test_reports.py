...
def test_utc_start_of_day_uses_utc_calendar_date_not_local():
    ...
    assert reports_routes.utc_start_of_day(behind) == datetime(2024, 3, 10, tzinfo=timezone.utc)
    ...
    assert reports_routes.utc_start_of_day(ahead) == datetime(2024, 3, 9, tzinfo=timezone.utc)
...
def test_todays_movements_passes_aware_utc_midnight_to_query(client, fake_db, monkeypatch):
