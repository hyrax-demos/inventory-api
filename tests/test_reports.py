...
def test_reserved_value_prices_with_same_tenant_item(client, fake_db):
    # Tenant B's item shares the SKU and warehouse and is seeded first, so an
    # unscoped join would price tenant A's reservation with B's price.
    fake_db.add_item(
...
def test_reserved_value_prices_tenant_b_with_its_own_item(client, fake_db):
    fake_db.add_item(
        sku="WIDGET", warehouse_id="w1", quantity=100, price=2.0, tenant_id="tenant-a"
    )
    fake_db.add_item(
    ...
    assert lines[0]["reserved_value"] == 200.0
