...
class FakeDB:
    ...
    def returning_rows(self, sql: str, params: tuple = ()) -> list[tuple]:
        """Rows a ``... RETURNING`` write would return, without applying it.

        Rows are tuples, matching psycopg2's default (non-dict) cursor.
        """
        if sql.startswith("DELETE FROM reservations") and "RETURNING" in sql:
            order_id, tenant_id = params
            cols = [c.strip() for c in sql.split("RETURNING", 1)[1].split(",")]
            return [
                tuple(r[c] for c in cols)
                for r in self.reservations
                if r["order_id"] == order_id and r["tenant_id"] == tenant_id
            ]
        raise AssertionError(f"FakeDB.returning_rows: unrecognized query: {sql!r}")
...
class _FakeCursor:
    ...
    def execute(self, sql: str, params: tuple = ()) -> None:
        if sql.lstrip().upper().startswith("SELECT"):
            ...
            self._rows = self._fake_db.returning_rows(sql, params)
