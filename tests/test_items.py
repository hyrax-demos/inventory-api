...
def test_get_stock_cache_miss_for_other_tenant_is_not_served_from_cache(
    client, fake_db
):
    fake_db.add_item(sku="WIDGET", warehouse_id="w1", quantity=10, tenant_id="tenant-a")
    ra = client.get(
...
def test_reserve_stock_is_reflected_on_next_stock_read(client, fake_db):
    ...
    assert (
        client.get("/items/WIDGET/stock", params=params_w1, headers=TENANT_A).json()[
            "quantity"
        ]
        == 10
    )
    assert (
        client.get("/items/WIDGET/stock", params=params_w2, headers=TENANT_A).json()[
            "quantity"
        ]
        == 20
    )
